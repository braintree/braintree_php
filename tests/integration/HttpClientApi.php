...
class HttpClientApi extends Http
{
    ...
    {
        return $this->_doUrlRequest($httpVerb,
            $this->_config->baseUrl() . "/merchants/" . $this->_config->merchantId() . $path, $requestBody);
    }
    ...
    public function nonce_for_new_card($options)
    {
        ...
        $clientToken = json_decode(TestHelper::decodedClientToken($clientTokenOptions));
    ...
    {
        $clientToken = json_decode(TestHelper::decodedClientToken());
        $options["authorization_fingerprint"] = $clientToken->authorizationFingerprint;
